...
import {
  Users, BookOpen, ClipboardList, Award, BarChart3, FileText,
  Shield, UserCheck, GraduationCap, TrendingUp, Clock, CheckCircle
} from "lucide-react";
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      {user.role === "super_admin" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Admin" value={12} change="+2 bulan ini" changeType="up" icon={Shield} gradient="primary" delay={0} />
          <StatCard title="Total Dosen" value={48} change="+5 bulan ini" changeType="up" icon={UserCheck} gradient="accent" delay={80} />
          <StatCard title="Total Mahasiswa" value="1,234" change="+120 bulan ini" changeType="up" icon={Users} gradient="success" delay={160} />
          <StatCard title="Total Kursus" value={86} change="+8 bulan ini" changeType="up" icon={BookOpen} gradient="warm" delay={240} />
        </div>
      )}

      {user.role === "admin" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total User" value="1,294" change="+127 bulan ini" changeType="up" icon={Users} gradient="primary" delay={0} />
          <StatCard title="Kursus Aktif" value={52} change="+3 minggu ini" changeType="up" icon={BookOpen} gradient="accent" delay={80} />
          <StatCard title="Menunggu Persetujuan" value={8} icon={Clock} gradient="warm" delay={160} />
          <StatCard title="Kategori" value={15} icon={FileText} gradient="success" delay={240} />
        </div>
      )}

      {user.role === "dosen" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Kursus Saya" value={6} icon={BookOpen} gradient="primary" delay={0} />
          <StatCard title="Total Mahasiswa" value={182} change="+12 minggu ini" changeType="up" icon={Users} gradient="accent" delay={80} />
          <StatCard title="Tugas Belum Dinilai" value={23} icon={ClipboardList} gradient="warm" delay={160} />
          <StatCard title="Kuis Aktif" value={4} icon={Award} gradient="success" delay={240} />
        </div>
      )}

      {user.role === "mahasiswa" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Kursus Diikuti" value={5} icon={BookOpen} gradient="primary" delay={0} />
          <StatCard title="Tugas Pending" value={3} icon={ClipboardList} gradient="warm" delay={80} />
          <StatCard title="Kuis Mendatang" value={2} icon={Award} gradient="accent" delay={160} />
          <StatCard title="Rata-rata Nilai" value="85.4" change="+2.1 dari semester lalu" changeType="up" icon={TrendingUp} gradient="success" delay={240} />
        </div>
      )}
